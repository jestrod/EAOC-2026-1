...
% Inference interface and defuzzification with COG method
for i=1:5
    for j=1:5
        
        % AND operator to compute implication certainty
        mu_premise = min(mu_e(i),mu_de(j));

        %COG method
        area_implied = area_implied_fs(mu_premise,0.5);
        mu_out = rules(i,j)* area_implied;
        num = num + mu_out;
        den = den + area_implied;
